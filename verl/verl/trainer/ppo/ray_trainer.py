...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.PSFT:
        data.batch["advantages"] = torch.ones_like(data.batch["response_mask"], dtype=torch.float32) * data.batch["response_mask"]
        data.batch["returns"] = torch.ones_like(data.batch["response_mask"], dtype=torch.float32) * data.batch["response_mask"]
    else:
